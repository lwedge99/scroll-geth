...
func (t *sentioPrestateTracer) CaptureEnd(output []byte, gasUsed uint64, _ time.Duration, err error) {
...
func (t *sentioPrestateTracer) CaptureStateAfter(pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
}
