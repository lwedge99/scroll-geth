...
func (t *sentioTracer) CaptureEnd(output []byte, usedGas uint64, _ time.Duration, err error) {
	t.callstack[0].EndIndex = t.index
...
func (t *sentioTracer) CaptureStateAfter(pc uint64, op vm.OpCode, gas, cost uint64, scope *vm.ScopeContext, rData []byte, depth int, err error) {
}
